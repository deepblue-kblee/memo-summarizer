# 📚 AI Reference Library Index

> **목적**: AI 에이전트들이 작업 시 참조할 수 있는 검증된 지식 베이스

## 📋 참조 문서 목록

### **AI 작업 연속성 관리**
- [`AI_CONTEXT_MANAGEMENT_RESEARCH.md`](AI_CONTEXT_MANAGEMENT_RESEARCH.md) - 종합 조사 보고서 (업계 베스트 프랙티스)
- [`BEST_PRACTICES.md`](BEST_PRACTICES.md) - 핵심 베스트 프랙티스 요약
- [`TOOLS_AND_PATTERNS.md`](TOOLS_AND_PATTERNS.md) - 검증된 도구와 패턴 모음
- [`README.md`](../../README.md) - 실제 적용 가이드

### **문서 작성 가이드**
- [`MARKDOWN_STYLE_GUIDE.md`](MARKDOWN_STYLE_GUIDE.md) - 마크다운 문서 작성 규칙 (이모지 사용 자제)

## 🔍 빠른 참조

### **파일 크기 관리 문제**
→ [`BEST_PRACTICES.md#파일-크기-관리`](BEST_PRACTICES.md#파일-크기-관리)

### **AI 세션 간 컨텍스트 전달**
→ [`TOOLS_AND_PATTERNS.md#핸드오프-프로토콜`](TOOLS_AND_PATTERNS.md#핸드오프-프로토콜)

### **문서 구조 최적화**
- [`README.md#메모-처리-파이프라인-구조`](../../README.md#메모-처리-파이프라인-구조)

### **자동화 스크립트**
→ [`TOOLS_AND_PATTERNS.md#자동화-도구`](TOOLS_AND_PATTERNS.md#자동화-도구)

## 📅 업데이트 기록

- **2026-03-23**: AI 작업 연속성 관리 조사 결과 추가
- 향후 새로운 조사나 베스트 프랙티스 발견 시 지속 업데이트

## 🤖 AI 에이전트 사용 지침

1. **문제 해결 시**: INDEX.md에서 관련 주제 검색
2. **새 작업 시작**: BEST_PRACTICES.md에서 표준 패턴 확인
3. **구현 시**: IMPLEMENTATION_GUIDE.md에서 구체적 방법 참조
4. **도구 선택**: TOOLS_AND_PATTERNS.md에서 검증된 솔루션 확인

---

> **📝 참고**: 이 라이브러리는 실제 검증된 업계 사례를 기반으로 하며, 지속적으로 업데이트됩니다.